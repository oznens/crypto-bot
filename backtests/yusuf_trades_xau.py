...
def add_features(m15):
    ...
    # Structural/trend bias: EMA alignment + location relative to 20-bar midpoint.
    mid = (h1.hh20 + h1.ll20) / 2
    h1['bias'] = 0
    h1.loc[(h1.ema20 > h1.ema50) & (h1.close > mid), 'bias'] = 1
    h1.loc[(h1.ema20 < h1.ema50) & (h1.close < mid), 'bias'] = -1
    # shift one H1 bar so M15 can only use completed H1 information
...
def candidate_setups(x):
    setups=[]
    # Event candle must sweep liquidity, displace, and close through 3-bar micro structure.
    for i in range(25, len(x)-20):
        r=x.iloc[i]
        if not math.isfinite(r.atr) or r.atr <= 0 or not math.isfinite(r.vol_ma20):
            continue
        vol_ok = r.tick_volume >= 1.05*r.vol_ma20
        if not vol_ok:
            continue
        ...
        # Last opposite candle in previous 5 bars defines proximal S/D zone.
        z=None
...
def run_variant(x, setups, rr=2.0, session=False):
    ...
    for i,side,zl,zh,a in setups:
        if i <= busy_until:
            continue
        # First retest only, up to 12 M15 bars after displacement.
        entry_idx=None
        entry=None
        ...
        for k in range(entry_idx, min(entry_idx+97,len(x))): # max 24h
            bar=x.iloc[k]
            hit_sl = bar.low <= stop if side==1 else bar.high >= stop
            hit_tp = bar.high >= target if side==1 else bar.low <= target
            if hit_sl and hit_tp:
                result=-1.0; exit_idx=k; break  # pessimistic same-bar assumption
            if hit_sl:
                result=-1.0; exit_idx=k; break
            if hit_tp:
                result=rr; exit_idx=k; break
        if result is None:
            # time exit at close, normalized to R and capped between -1 and rr
            k=min(entry_idx+96,len(x)-1)
            px=float(x.iloc[k].close)
            result=max(-1.0,min(rr, side*(px-entry)/risk))
            exit_idx=k
        trades.append({
            'entry_time':str(x.index[entry_idx]),'exit_time':str(x.index[exit_idx]),
            'side':'LONG' if side==1 else 'SHORT','entry':entry,'stop':stop,'target':target,'R':result
        })
        busy_until=exit_idx
    if not trades:
    ...
    stats={
        'trades':int(len(rs)), 'win_rate_pct':round(100*wins/len(rs),2),
        'net_R':round(float(rs.sum()),2), 'avg_R':round(float(rs.mean()),4),
        'profit_factor':round(float(gross_win/gross_loss),3) if gross_loss else None,
        'max_drawdown_R':round(float(dd.min()),2),
        'wins':int(wins),'losses':int(losses),
        'start':trades[0]['entry_time'],'end':trades[-1]['exit_time']
    }
    return trades, stats
