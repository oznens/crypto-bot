...
assert.equal(item.setup.entry,100);
assert.equal(item.setup.stop,98);

const fallback = J.upsert(rows, {
  symbol:'ETH_USDT',tf:'1h',model:'Unicorn',anchor:456,
  status:'WATCHING',decision:'SETUP_FOUND',reason:'ENTRY_PENDING',
  marketPrice:2500,
  setup:{side:'LONG',model:'Unicorn',confidence:80,entry:2400,stop:2400,tps:[2600],rr:2}
}, { now:1100, limit:100 });
assert.equal(fallback.setup.entry,2500);
assert.equal(fallback.setup.stop,2400);

J.upsert(rows,{id:item.id,status:'REJECTED',decision:'NO_TRADE',reason:'TIMEFRAME_CONFLICT'},{now:2000});
assert.equal(rows.find(x=>x.id===item.id).firstSeenAt,1000);
assert.equal(rows.find(x=>x.id===item.id).lastSeenAt,2000);
assert.equal(rows.find(x=>x.id===item.id).reason,'TIMEFRAME_CONFLICT');
...
assert.equal(rows.find(x=>x.id===item.id).status,'OPENED');
assert.equal(rows.find(x=>x.id===item.id).tradeId,'t1');
...
assert.equal(rows.find(x=>x.id===item.id).status,'CLOSED');
